...
const NewCarousel = ({
  ...
  const [isActionEnabled, setIsActionEnabled] = React.useState(true)
   ...
   */
  const nextItem = () => {
    if ((isRepeating || currentIndex < (length - show)) && isActionEnabled) {
   ...
   */
  const previousItem = () => {
    if ((isRepeating || currentIndex > 0) && isActionEnabled) {
      setCurrentIndex(prevState => prevState - 1)
    }
    setIsActionEnabled(false)
  }
  ...
  React.useEffect(() => {
    const interval = autoMove ? setInterval(() => {
        nextItem()
    }, autoMove) : null
  ...
  const extraPreviousItems = React.useMemo(() => {
    let output = []
    const limit = length - show <= 1 ? show : show - 1
  ...
  return (
    <div
      data-testid="new-carousel-container"
      className={`new-carousel-container ${containerClassName || ''}`}
      {...containerProps}
    >
      <div
      ...
      >
        {
          (isRepeating || currentIndex > 0) ?
            renderPreviousButton ?
            renderPreviousButton(previousItem, 'left-arrow-button', !isActionEnabled)
            :
            <button
        ...
        >
          <div
            ...
            {...contentProps}
            style={{
              transform: `translateX(${transformCalculation ? transformCalculation(currentIndex, show) : (-currentIndex * (100 / show))}%)`,
        ...
        </div>
        {
          (isRepeating || currentIndex < (length - show)) ?
            renderNextButton ?
            renderNextButton(nextItem, 'right-arrow-button', !isActionEnabled)
            :
            <button
